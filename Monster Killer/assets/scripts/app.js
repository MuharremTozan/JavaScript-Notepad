...
function getMaxLifeValues() {
   ...
   if (isNaN(chosenMaxLife) || chosenMaxLife <= 0) {
      throw { message: 'Invalid user input, not is a number' };
   }
   return parsedValue;
}
...
function writeToLog(eventf, valuef, monsterHealth, playerHealth) {
   let logEntry;
   logEntry = {
      event: eventf,
      value: valuef,
   ...
   // if (eventf === LOG_EVENT_PLAYER_ATTACK) {
   //    logEntry.target = 'MONSTER';
   // }
   // else if (eventf === LOG_EVENT_PLAYER_STRONG_ATTACK) {
   //    logEntry.target = 'MONSTER';
   // }
   // else if (eventf === LOG_EVENT_MONSTER_ATTACK) {
   //    logEntry.target = 'PLAYER';
   // }
   // else if (eventf === LOG_EVENT_PLAYER_HEAL) {
   //    logEntry.target = 'PLAYER';
   // }
}
